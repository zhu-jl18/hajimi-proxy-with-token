...
### 访问时浏览器弹出“登录框”（Basic Auth）怎么办？

现象：打开站点就出现带“用户名/密码”的登录弹窗。

原因：这是浏览器对 `WWW-Authenticate: Basic ...` 的自动响应。可能来自：
- 站点开启了 Netlify 的 Password protection；或
- 上游返回了 `WWW-Authenticate`（例如 401 时），而代理把它原样透传给了浏览器。

现在的行为：
- 本项目已在两处进行了处理：
	- 当你未带 `PROXY_TOKEN` 访问时，返回 `401` 且使用 `WWW-Authenticate: Bearer ...` 提示（不会触发浏览器弹窗）。
	- 对上游响应我们会去除 `WWW-Authenticate` 头，避免弹窗。

如果你仍然看到弹窗，请：
- 确认站点的 Password protection 处于关闭状态（免费账户默认是关闭）；
- 用隐身窗口再次访问，或清理该站点的缓存凭据；
- 在 DevTools → Network 查看响应头里是否还有 `WWW-Authenticate`；
- 或用命令行验证：

```powershell
curl -i https://<你的站点>.netlify.app/
```

若仍有问题，把响应头贴给我，我来帮你定位。
